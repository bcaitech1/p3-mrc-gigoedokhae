...
def run_sparse_embedding():
    retriever = SparseRetrieval(tokenize_fn=tokenize,  # mecab.morphs(text)
...
def run_mrc(data_args, training_args, model_args, datasets, tokenizer, model):
    ...
    # 에러가 발생하는지 점검하고 만약 이전에 학습된 checkpoint가 있다면 불러온다.
    ...
    # 데이터를 뽑아서 배치로 만든다음 모델에 다음 입력으로 준다.
    data_collator = (
        DataCollatorWithPadding(
    ...
    metric = load_metric("squad")  # EM, F1 스코어
