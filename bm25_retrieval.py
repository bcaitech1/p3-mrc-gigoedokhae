...
def tokenize(sentence):
    tokenizer = Mecab('/opt/conda/mecab/dic')
...
def retrieve(dataset, k):
    data_path = '/opt/ml/pycharm/bm25_mecab_wiki_data.pickle'
...
def make_test_set():
    ...
    df = retrieve(dataset, 50)

    data = DatasetDict({'validation': Dataset.from_pandas(df)})
    data.save_to_disk('/opt/ml/test_bm25plus_mecab_top50')
