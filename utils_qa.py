...
mecab = Mecab(dicpath='/opt/conda/mecab/dic/')
...
#for top20
def postprocess_qa_predictions_top20(
    examples,
    features,
    predictions: Tuple[np.ndarray, np.ndarray],
    version_2_with_negative: bool = False,
    ...
    for i, feature in enumerate(features):
        features_per_example[example_id_to_index[feature["example_id"]]].append(i)
        all_context_id.append(feature['context_id'])
    ...
    for example_index, example in enumerate(tqdm(examples)):
        ...
        for feature_index in feature_indices:
            ...
            #for top20
            context_id = all_context_id[feature_index]
            # This is what will allow us to map some the positions in our logits to span of texts in the original
            # context.
            offset_mapping = features[feature_index]["offset_mapping"]
            # Optional `token_is_max_context`, if provided we will remove answers that do not have the maximum context
        ...
        for pred in predictions:
            offsets = pred.pop("offsets")
            context_id = pred.pop('context_id')
            pred["text"] = context[context_id][offsets[0] : offsets[1]]
